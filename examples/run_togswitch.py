...
n = 2000
d = 1
theta_star = np.array([22.0, 12.0, 4.0, 4.5, 325.0, 0.25, 0.15])
...
if sample_data_bool:
    for j in range(R):
        theta_star_rep = np.array(
            [np.log(22), np.log(12), 4.0, 4.5, np.log(325), np.log(0.25), 0.15]
        )  # reparametrise
...
plot_posterior_marg_tsols(
    B, thetas_wabc, thetas_mmd, theta_star, y, fname, save_fig=True
)
