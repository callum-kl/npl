...
results_path = "./../data/G_and_k_model/results/"
plots_path = "./../data/G_and_k_model/plots"
...
times = []
summary_stats = np.zeros(
    (R, outl, p, 4)
)  # collect mean, median, mode, st.dev for each bootstrap sample
for j in range(R):
    print("-----Run ", j)
    for n_cont in range(outl):
        ...
        np.savetxt(
            results_path + "thetas_mmd_outl_{}_run_{}.txt".format(n_cont, j),
            sample,
        )

# np.savetxt(results_path + "summary_stats.txt", summary_stats)
np.savetxt(results_path + "cpu_times.txt", times)
#%%
...
for i in range(outl):
    for j in range(p):
        sample = np.loadtxt(
            results_path + "thetas_mmd_outl_{}_run_{}.txt".format(i, r)
        )
...
for n_cont in range(outl):
    for i, name in enumerate(names):
        df = pd.read_csv(
            results_path + "WABC/thetas_wabc_outl_{}_run_{}new2.csv".format(n_cont, r)
        )
        thetas_wabc[i, n_cont, :] = df[name]
thetas_wabc[3, :, :] = np.log(
    thetas_wabc[3, :, :]
)  # last parameter for gandk is exp(k)
...
outl = 3
pal = sns.cubehelix_palette(start=0.5, rot=-0.75, n_colors=B)
...
plt.plot(
    n_range,
    np.sqrt(np.mean(mmd_dists, axis=1)),
    label="$\sqrt{\mathbb{E}[\widehat{MMD}^2]}$",
    marker=11,
    color="#0072B2",
)
plt.fill_between(
    n_range,
    np.sqrt(np.mean(mmd_dists, axis=1)) - np.sqrt(np.std(mmd_dists, axis=1)),
    np.sqrt(np.mean(mmd_dists, axis=1)) + np.sqrt(np.std(mmd_dists, axis=1)),
    alpha=0.1,
)
...
for i, a in enumerate(alphas):
    ...
    for j in range(outl):
        ...
        npl_ = NPL_prior.npl_prior(
            X, B, m, s, p, l, model=model, a=a, T=T, model_name=model_name
        )
        npl_.draw_samples()
        sample = npl_.sample
        ...
        np.savetxt(
            results_path
            + "alpha_exp_theta_outl_{}_alpha_{}_T_{}_2.txt".format(j, a, T),
            sample,
        )
...
for j in range(outl):
    plt.plot(
        alphas, mses[:, j], marker="o", label=r"$\epsilon$ = {}".format(j * 5 / 100)
    )
...
# Densities plots
pal = sns.cubehelix_palette(start=0.5, rot=-0.75, n_colors=len(alphas))
...
for j, ax in enumerate(ax_array):
    ...
    for i in range(len(alphas)):
        ...
        ax = sns.kdeplot(
            sample.flatten(), color=pal[i], ax=ax
        )  # label='n={}'.format(n), #, bw_adjust=1
...
for i, T in enumerate(Ts):
    ...
    for j in range(outl):
        ...
        theta_samples[i, j, :, :] = sample
        np.savetxt(
            results_path + "T_exp_theta_outl_{}_alpha_{}_T_{}_2.txt".format(j, a, T),
            sample,
        )
...
# Densities plots
pal = sns.cubehelix_palette(start=0.5, rot=-0.75, n_colors=len(Ts))
fig, ax_array = plt.subplots(1, outl, figsize=(20, 6))
for j, ax in enumerate(ax_array):
    ax = sns.kdeplot(datasets[0, :].flatten(), ax=ax, linestyle="--", color="black")
    for i in range(len(Ts)):
...
# Range of ls
ls = np.array(
    [10 ** (-1), 10 ** (-0.5), 10**0, 10 ** (0.5), 10**1, 10 ** (1.5), 10**2]
)
...
for i, l in enumerate(ls):
    ...
    for j in range(outl):
        ...
        theta_samples[i, j, :, :] = sample
        np.savetxt(
            results_path + "alpha_exp_theta_outl_{}_l_{}_new.txt".format(j, l), sample
        )
...
# Densities plot
pal = sns.cubehelix_palette(start=0.5, rot=-0.75, n_colors=len(ls))
fig, ax_array = plt.subplots(1, outl, figsize=(20, 6))
for j, ax in enumerate(ax_array):
    ax = sns.kdeplot(datasets[0, :].flatten(), ax=ax, linestyle="--", color="black")
    for i in range(len(ls)):
