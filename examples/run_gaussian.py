...
from plot_functions import (
    SeabornFig2Grid,
    plot_gauss_4d_mmd_vs_was,
    plot_gauss_4d,
    plot_posterior_marginals_mmd_vs_mabc,
)
import NPL
import models
import numpy as np
...
for j in range(R):
    ...
    for n_cont in range(outl):
        ...
        np.savetxt(
            results_path
            + "NPL_MMD/thetas_mmd_outl_{}_run_{}_dim_{}.txt".format(n_cont, j, d),
            sample,
        )
        np.savetxt(
            results_path
            + "NPL_WLL/thetas_wll_outl_{}_run_{}_dim_{}.txt".format(n_cont, j, d),
            sample,
        )
...
for i in range(outl):
    for j in range(p):
        sample = np.loadtxt(
            results_path
            + "NPL_MMD/thetas_mmd_outl_{}_run_{}_dim_{}.txt".format(i, r, d)
        )
        sample_wll = np.loadtxt(
            results_path
            + "NPL_WLL/thetas_wll_outl_{}_run_{}_dim_{}.txt".format(i, r, d)
        )
        sample_was = np.loadtxt(
            results_path
            + "NPL_WAS/thetas_mmd_outl_{}_run_{}_dim_{}_.txt".format(i, r, d)
        )
        if p > 1:
...
for n_cont in range(outl):
    for i, name in enumerate(names):
        df = pd.read_csv(
            results_path
            + "WABC/thetas_wabc_outl_{}_run_{}_dim_{}.csv".format(n_cont, r, d)
        )
...
for n_cont in range(outl):
    for i, name in enumerate(names):
        df = pd.read_csv(
            results_path
            + "WABC/thetas_mabc_outl_{}_run_{}_dim_{}.csv".format(n_cont, r, d)
        )
        df = df[0:500]
...
axes2 = plot_gauss_4d_3(
    B, thetas_mmd, thetas_was, theta_star, 2, fname1, save_fig=False
)
...
axes = plot_posterior_marginals_mmd_vs_mabc(
    B, thetas_mmd, thetas_mabc, theta_star, 2, fname2
)
